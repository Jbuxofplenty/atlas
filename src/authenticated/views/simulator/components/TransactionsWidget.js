...
function TransactionsWidget(props) {
  ...
  useEffect(() => {
    var height = document.getElementById('transactions-table-container').clientHeight;
    ...
    >
      <div className={`d-flex justify-content-center w-100`}>
        <GridContainer justify="center" className={`${s.gridContainer}`}>
          <h5 className={`${s.portfolioHeader}`}>Completed Transactions</h5>
          <div className={`${s.rounded}`}></div>
          <div id="transactions-table-container" className="w-100">
            <TransactionsTable transactions={props.portfolio.transactions} />
          </div>
        </GridContainer>
      </div>
    </Widget>
...
export default connect(mapStateToProps, mapDispatchToProps)(withRouter(TransactionsWidget));
